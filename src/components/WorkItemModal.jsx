...
const WorkItemModal = ({ item, onClose, onUpdated }) => {
    ...
    const [showAssocModal, setShowAssocModal] = useState(null) // 'plan' | 'tc' | null
...
`)
    ...
    const handleAssociate = async (type, targetId) => {
        ...
        try {
            ...
            await fetchAssociations() // Refrescar en tiempo real
                                ...
                                <div className="bg-white rounded-xl border border-gray-200 w-full max-w-md">
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="max-h-64 overflow-y-auto space-y-1">
                                            ...
                                            {showAssocModal === 'plan' && allPlans.filter(p =>
                                                p.name.toLowerCase().includes(assocSearch.toLowerCase())).length === 0 && (
                                                    <p className="text-xs text-gray-400 text-center py-4">No hay planes disponibles</p>
                                                )}
                                            {showAssocModal === 'tc' && allTCs.filter(tc =>
                                                tc.title.toLowerCase().includes(assocSearch.toLowerCase())).length === 0 && (
                                                    <p className="text-xs text-gray-400 text-center py-4">No hay test cases disponibles</p>
                                                )}
                                        </div>
                                    </div>
                                </div>
